...
| Evaluation RAGAS + `RESULT.md` | Chưa chạy 4 metric và A/B | — | Blocked |
...
- **Test tự động:** `pytest -q` → 19/20 pass. `tests/test_contracts.py` pass toàn bộ 15/15. Cái còn lại là `test_evaluation_report_is_completed`, fail vì `RESULT.md` chưa có số liệu.
